/***************************************************************************
 $RCSfile: chameleon.c,v $
                             -------------------
    cvs         : $Id: chameleon.c,v 1.1 2003/06/07 21:07:48 aquamaniac Exp $
...
ERRORCODE Chameleon_Init() {
  ...
  if (!Error_IsOk(err))
    return err;
  err=IPCMessage_ModuleInit();
  if (!Error_IsOk(err))
    return err;
  err=LibLoader_ModuleInit();
  if (!Error_IsOk(err))
    return err;
...
ERRORCODE Chameleon_Fini() {
    ...
    err=0;
    //add here more modules
    if (!Error_IsOk(LibLoader_ModuleFini())) {
      err=Error_New(0,
		    ERROR_SEVERITY_ERR,
		    0,
		    ERROR_COULD_NOT_UNREGISTER);
      DBG_ERROR("Chameleon_Fini: "
		"Could not deinitialze module LibLoader");
    }
#ifdef HAVE_SSL
    if (!Error_IsOk(Cryp_ModuleFini())) {
      err=Error_New(0,
		    ERROR_SEVERITY_ERR,
		    0,
		    ERROR_COULD_NOT_UNREGISTER);
      DBG_ERROR("Chameleon_Fini: "
		"Could not deinitialze module Cryp");
    }
#endif
    if (!Error_IsOk(IPCMessage_ModuleFini())) {
      err=Error_New(0,
		    ERROR_SEVERITY_ERR,
		    0,
		    ERROR_COULD_NOT_UNREGISTER);
      DBG_ERROR("Chameleon_Fini: "
		"Could not deinitialze module IPCMessage");
    }
    if (!Error_IsOk(Socket_ModuleFini())) {
      err=Error_New(0,
		    ERROR_SEVERITY_ERR,
		    0,
		    ERROR_COULD_NOT_UNREGISTER);
...
void Chameleon_DumpString(const char *s, int l) {
  unsigned int i;
  unsigned int j;
  unsigned int pos;
